                    ...
                    <tr>
                      ...
                      <tr>
                        ...
                        <td colspan="12">
                            ...
                            @if(!empty($orders[0]->userdetails->pay_type))
                                @php
                                $type='';
                                if($orders[0]->userdetails->pay_type=='Prepaid') $type='Online Payment';
                     ...
                     <tr>
                            ...
                            @php ++$key;@endphp
                              <tr>
                                  @if($key==1)
                                  <td rowspan="{{count($orders[0]->orderproducts)}}"></td>
                                  @endif
                                  <td class="text-left" style="width:7%;">{{$val->product_name}}</td>
                                  <td class="text-left" style="width:7%;">{{$val->quantity}}</td>
                              <td class="text-left" style="width:7%;"> {{$val->price}}</td>
                                  <td class="text-left" style="width:7%;">@php $sum+=$val->total_price; @endphp {{$val->total_price}}</td>
                              </tr>

                              @if($key==count($orders[0]->orderproducts))
                              <td ></td>
                              <td ></td>
                              <td ></td>
                      ...
                      <tr>
                        ...
                        <tr>
                            <th>Deliver Date</th>
                            <td colspan="12">@if(!empty($orders[0]->delivery_date)){{$orders[0]->delivery_date}}@else{{'--'}}@endif</td>
                          </tr>
                          <tr>
                            <th>Delivery Time</th>
                            <td colspan="12">@if(!empty($orders[0]->delivery_time)){{$orders[0]->delivery_time}}@else{{'--'}}@endif</td>
                          </tr>
                          <tr>
                      ...
                      <input type="hidden" name="user_id" id="user_id"   value="@if(!empty($orders[0]->user_id)){{$orders[0]->user_id}}@endif" />
                      <input type="hidden" name="user_id" id="user_id"   value="@if(!empty($orders[0]->user_id)){{$orders[0]->user_id}}@endif" />
                      <input type="hidden" name="order_id" id="order_id" value="@if(!empty($orders[0]->order_id)){{$orders[0]->order_id}}@endif"/>
                                <th>Status</th>
                                <td style="width: 12%;">
                                    <select name="status" id="status" class="form-control">
                                        <option value="pending" @if(!empty($orders[0]->status) && $orders[0]->status=='pending'){{'selected'}}@endif>Pending</option>
